...
class TriDiagonalMatrix : public AbstractMatrix<T>
{
  ...
  public:
    ...
    /**
     * Purpose: Bracket operator - Peek at contents of the Matrix
     * Pre: Parameter x must be in the interval [0, rows)
     * Param: x - The row to be grabbed
     * Post: If x is not in the interval [0, rows), an invalid_argument exception is thrown
    ...
    /**
     * Purpose: Virtual print function - Prints the state of the TDM
     * Pre: Parameter out must be connected to a valid stream
     * Param: out - The ostream to output our TDM to
     * Post: The TDM will be output to the stream out is connected to
     * Return: out
     */
    virtual std::ostream& output(std::ostream& out) const;

    /**
     * Purpose: Virtual Gaussian Elimination function
     * Pre: Parameter bVector must have the same length as the calling object has columns
     *      Template type T must have operator<<(ostream, T) defined
     * Param: bVector - The "B" Vector in the equation Ax = b
     * Post: If the solver divides by zero, a domain_error exception will be thrown
     * Return: The answer to the equation Ax = b
     */
